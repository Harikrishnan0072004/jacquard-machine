...
const int LOG_SETTING_ADDRESS = 34;
...
void setup() {
  ...
  for (int passwordIndex = 0; passwordIndex < 6; passwordIndex++) {
    byte passwordByte = EEPROM.read(passwordIndex);
    if (passwordByte < '0' || passwordByte > '9') {
      passwordValid = false;
      break;
    }
  }
  if (!passwordValid) {
    const char defaultPassword[] = "000001";
    for (int passwordIndex = 0; passwordIndex < 6; passwordIndex++) {
      EEPROM.write(passwordIndex, defaultPassword[passwordIndex]);
    }
    EEPROM.commit();
  }
  byte storedLogSetting = EEPROM.read(LOG_SETTING_ADDRESS);
  if (storedLogSetting > 1) {
    storedLogSetting = 1;
    EEPROM.write(LOG_SETTING_ADDRESS, storedLogSetting);
    EEPROM.commit();
  }
...
{
  logEnabled = enabled;
  EEPROM.write(LOG_SETTING_ADDRESS, enabled ? 1 : 0);
  EEPROM.commit();
  Serial2.print("ABG");
  Serial2.print(enabled ? '1' : '0');
  Serial2.print("#{}");
  Serial2.flush();
  if (enabled) {
    Serial1.begin(230400);
    Serial1.println("DEBUG LOG ON");
  } else {
    Serial1.end();
  }
}
...
void runningdisplay()
{
 byte ck12 = 0;
 byte ck13 = 0; 
 static byte displayedMode = 255;
 if (displayedMode != filerunningmode)
 {
  lcd.clear();
  displayedMode = filerunningmode;
 }
 lcd.noCursor(); 
 lcd.setCursor(0, 0);
 lcd.print("BD:-"); 
 ...
 s_name.trim();
 int dotIndex = s_name.lastIndexOf('.');
 if (dotIndex > 0) s_name.remove(dotIndex);
 if (s_name.length() > 14) s_name.remove(14);
 lcd.print(s_name); 
 lcd.setCursor(0, 1);
 lcd.print("     ");
 //delay(100); 
...
void runningdesign()
{
   // Force LCD refresh whenever returning from menu
   lcd.clear();
...
menu: 
 ...
 s_name = folderlist_m[foldernum];
 s_name.trim();
 if (s_name.length() > 13) s_name.remove(13);
 lcd.print(s_name);
 lcd.print("               ");
 if (mcnt < 4)
 {    
      ...
      case 2:
          Serial.println("case 2 start");  Serial.println(tempg1);
           ...
           //    snprintf(fbuf, sizeof(fbuf), "ABF%d#&", tempq);
            ...
            lcd.clear();
            lcd.setCursor(0, 1);
            lcd.print("Folder Selected:");
            lcd.setCursor(0, 2);
            lcd.print(folderlist_m[foldernum]);
            delay(500);
            goto menu;
      case 3:
           Editmenu();   
