...
byte sensor1rawstatus = HIGH, sensor2rawstatus = HIGH;
unsigned long sensor1transitiontime = 0, sensor2transitiontime = 0;
const unsigned long sensorDebounceMs = 5;
...
void setup() {
  ...
  delay(200);
  byte storedLogSetting = eeprom.read(34);
  if (storedLogSetting > 1)
  {
    storedLogSetting = 0;
    eeprom.write(34, storedLogSetting);
  }
  logEnabled = (storedLogSetting == 1);
  if (!logEnabled)
  {
    Serial.end();
  }
...
read1:
  ...
  byte sensor1reading = digitalRead(sensor1);
  if (sensor1reading != sensor1rawstatus)
  {
    sensor1rawstatus = sensor1reading;
    sensor1transitiontime = millis();
  }
  if ((millis() - sensor1transitiontime) >= sensorDebounceMs)
    sensor1status = sensor1rawstatus;
  ...
  byte sensor2reading = digitalRead(sensor2);
  if (sensor2reading != sensor2rawstatus)
  {
    sensor2rawstatus = sensor2reading;
    sensor2transitiontime = millis();
  }
  if ((millis() - sensor2transitiontime) >= sensorDebounceMs)
    sensor2status = sensor2rawstatus;
  ...
  while (true)
  {
    if (Serial2.available() > 0)
    {
      ...
      if (data3[0] == 65)          // for header 'A'
      {
        ...
        {
          if (data3[nc1 - 1] == 67 || data3[nc1 - 1] == 68 || data3[nc1 - 1] == 71) // 'C' check, 'D' commands, or 'G' log status
          {
            if (data3[nc1 - 1] == 67)
            {
              unsigned long checkPacketStart = millis();
              while (millis() - checkPacketStart < 1000)
              {
                if (Serial2.available() > 0)
                {
                  if ((byte)Serial2.read() == '}') break;
                }
              }
              Serial2.print('E');
              Serial2.print('J');
            ...
            {
              byte logValue = 0xFF;
              unsigned long logPacketStart = millis();
              while (millis() - logPacketStart < 1000)
              {
                if (Serial2.available() > 0)
                {
                  byte logByte = (byte)Serial2.read();
                  if (logValue == 0xFF && (logByte == '0' || logByte == '1'))
                  {
                    logValue = logByte - '0';
                  }
                  if (logByte == '}') break;
