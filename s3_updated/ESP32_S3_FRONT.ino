...
// The USB copy path (usbCopySingleFile/usbCopyFolder -> usbCopyOneFile) nests
// several calls deep and allocates a 1030-byte card buffer on the stack inside
// a loop; the default 8KB loop() task stack runs close to its limit there, so
// raise it here rather than risk silent stack-overflow corruption of globals
// (e.g. the keypad's key map) that only shows up as a dead key after a copy.
...
// Draws the static "TILT TECHNOLOGIES" header + title once, when a
// scrollable-list screen is first entered.
void drawScrollableMenuHeader(const String &title)
{
  tft.fillScreen(TFT_BLACK);
...
// Repaints only the list area (no full-screen clear, header untouched) --
// call this on every scroll step so navigation stays flicker-free, the
// same way drawMenu() does for the main menus.
void drawScrollableMenuList(String items[], int itemCount, int selectedIndex, int topIndex, int visibleRows)
{
  int x = 90;
  int y = 110;
  int rowHeight = 35;

  tft.fillRect(x - 15, y - 10, 390, visibleRows * rowHeight + 10, TFT_BLACK);
...
// Kept for compatibility with any other call sites: header + list in one call.
void drawScrollableMenu(const String &title, String items[], int itemCount, int selectedIndex, int topIndex, int visibleRows)
{
  drawScrollableMenuHeader(title);
  drawScrollableMenuList(items, itemCount, selectedIndex, topIndex, visibleRows);
}
...
byte data1[30];
static uint8_t usbPickBuffer[1030];
char pass[8];
char SysNo[8];
char out[11];
char file1[15];
...
// ==== TEMPORARY KEYPAD INTEGRITY CHECK -- remove after diagnosis ====
// keys[][]/pin_rows[]/pin_column[] are never written at runtime, so any
// mismatch here means something else overflowed into their memory.
const char keysExpected[ROW_NUM][COLUMN_NUM] = {
  {'1', '2', '3', '4'},
  {'5', '6', '7', '8'},
  {'9', '0', '*', 'L'},
  {'B', 'R', 'E', 'A'}
};
const byte pinRowsExpected[ROW_NUM]      = {47, 18, 5, 4};
const byte pinColumnExpected[COLUMN_NUM] = {38, 39, 3, 15};

void checkKeypadIntegrity(const char *whenLabel, bool quietOnSuccess = false) {
  bool ok = true;
  for (byte r = 0; r < ROW_NUM; r++) {
    for (byte c = 0; c < COLUMN_NUM; c++) {
      if (keys[r][c] != keysExpected[r][c]) {
        ok = false;
        Serial.print("KEYPAD CORRUPTION [");
        Serial.print(whenLabel);
        Serial.print("]: keys[");
        Serial.print(r);
        Serial.print("][");
        Serial.print(c);
        ...
        Serial.print("' (0x");
        Serial.print((byte)keys[r][c], HEX);
        Serial.print(") expected '");
        Serial.print(keysExpected[r][c]);
        Serial.println("'");
      }
    }
  }
  for (byte r = 0; r < ROW_NUM; r++) {
    if (pin_rows[r] != pinRowsExpected[r]) {
      ok = false;
      Serial.print("KEYPAD CORRUPTION [");
      Serial.print(whenLabel);
      Serial.print("]: pin_rows[");
      Serial.print(r);
      Serial.print("] = ");
      Serial.print(pin_rows[r]);
      Serial.print(" expected ");
      Serial.println(pinRowsExpected[r]);
    }
  }
  for (byte c = 0; c < COLUMN_NUM; c++) {
    if (pin_column[c] != pinColumnExpected[c]) {
      ok = false;
      Serial.print("KEYPAD CORRUPTION [");
      Serial.print(whenLabel);
      Serial.print("]: pin_column[");
      Serial.print(c);
      Serial.print("] = ");
      Serial.print(pin_column[c]);
      Serial.print(" expected ");
      Serial.println(pinColumnExpected[c]);
    }
  }
  if (ok && !quietOnSuccess) {
    Serial.print("Keypad integrity OK [");
    Serial.print(whenLabel);
    Serial.println("]");
  }
}
// ==== END TEMPORARY KEYPAD INTEGRITY CHECK ====
...
{Serial.begin(230400);
  ...
  // ==== TEMPORARY MEMORY LAYOUT CHECK -- remove after diagnosis ====
  Serial.println("=== MEMORY ADDRESS CHECK ===");
  Serial.printf("&data3[0]       = %p  (size %u)\n", (void*)&data3[0], (unsigned)sizeof(data3));
  Serial.printf("&folderlist_m[0]= %p  (size %u)\n", (void*)&folderlist_m[0], (unsigned)sizeof(folderlist_m));
  Serial.printf("&filelist_m[0]  = %p  (size %u)\n", (void*)&filelist_m[0], (unsigned)sizeof(filelist_m));
  Serial.printf("&filenum_m      = %p\n", (void*)&filenum_m);
  Serial.printf("&filenum1_m     = %p\n", (void*)&filenum1_m);
  Serial.printf("&foldernum      = %p\n", (void*)&foldernum);
  Serial.printf("&customKeypad   = %p  (size %u)\n", (void*)&customKeypad, (unsigned)sizeof(customKeypad));
  Serial.printf("&keys[0][0]     = %p  (size %u)\n", (void*)&keys[0][0], (unsigned)sizeof(keys));
  Serial.printf("&pin_rows[0]    = %p  (size %u)\n", (void*)&pin_rows[0], (unsigned)sizeof(pin_rows));
  Serial.printf("&pin_column[0]  = %p  (size %u)\n", (void*)&pin_column[0], (unsigned)sizeof(pin_column));
  Serial.printf("&data2[0]       = %p  (size %u)\n", (void*)&data2[0], (unsigned)sizeof(data2));
  Serial.printf("&data1[0]       = %p  (size %u)\n", (void*)&data1[0], (unsigned)sizeof(data1));
  Serial.printf("&usbFolderList[0]= %p (size %u)\n", (void*)&usbFolderList[0], (unsigned)sizeof(usbFolderList));
  Serial.printf("&usbFileList[0] = %p  (size %u)\n", (void*)&usbFileList[0], (unsigned)sizeof(usbFileList));
  Serial.println("=== END MEMORY ADDRESS CHECK ===");
  // ==== END TEMPORARY MEMORY LAYOUT CHECK ====
...
//EEPROM.begin(255);
 ...
 masterverify();
  ...
  if (totalfolder_m == 0)
  {
    ...
    tilt_tech();
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
    tft.drawString("Folder Error!", 35, 70, 4);
    tft.drawString("Keep 2 Folders", 35, 110, 4);
    tft.drawString("In SDCard", 35, 150, 4);
    delay(2000);
  }
...
void loop()
{
  checkKeypadIntegrity("loop top", true);
  Running_Page();
  delay(200);
  Main_menu();
}
             ...
             if (data3[2] == 76)       // for 'L'-List of file and folder             
             {
              if (data3[3] == 75)     // for 'K'-Folder
              {
                ...
                if (tempx >= 0 && tempx < 50) {
                  ...
                  Serial.print("WARNING: folder index out of range: ");
              ...
              if (data3[3] == 77)     // for 'M'-File
              {
                ...
                if (tempy >= 0 && tempy < 50) {
                  ...
                  Serial.print("WARNING: file index out of range: ");
...
void communicationchecking()
{
  ...
  out[0] = 'A';
  out[1] = 'B';
  out[2] = 'C';  // C for communication checking
  out[3] = '0';
  out[4] = 'F';
  out[5] = '{';
  out[6] = '}';
  ...
  while (millis() - attemptStart < 1000)
  {
    if (Serial2.available() > 0)
    {
      byte inbyte = (byte)Serial2.read();
      Serial.println(inbyte);
      if (previousByte == 'E' && inbyte == 'J')
      {
...
byte folder_selection(int folderCount)
{
  ...
  auto drawList = [&]() {
    // Repaint only the list area -- the header above is already on screen
    // and doesn't need to be touched again on every scroll step.
    tft.fillRect(x - 15, y - 10, width + 30, VISIBLE_ROWS * rowHeight + 10, TFT_BLACK);
...
bool confirmDelete(const String &itemType, const String &itemName)
{
  ...
  while (true)
  {
    ...
    if (customKey == '*') { delay(500); return false; }
  }
}
...
byte selectFileForDelete()
{
  ...
  drawScrollableMenuHeader("DELETE FILE");
  ...
  while (true)
  {
    if (needsRedraw)
    {
      ...
      drawScrollableMenuList(filelist_m, totalfile_m, selectedIndex, topIndex, VISIBLE_ROWS);
      needsRedraw = false;
    }
...
void edit_Mode()
{
 ...
 while (true)
 {
  ...
  if (customKey) 
  {
    ...
    if (customKey == 'E')     // Select
    {
     ...
     {        
      case 1:
            ...
            if (ls_name.equalsIgnoreCase(s_name))
              ...
              if (totalfile_m < 2)
              {
                uiClear();
                tilt_tech();
                uiLine(1, "Folder Warning!");
                uiLine(2, "Keep 2 Files");
                uiLine(3, "In Folder");
                delay(2000);
              }
              break;
      case 2:
      {
             USB();
              delay(300);
              unsigned long keyClearStart = millis();
              while (millis() - keyClearStart < 300)
              {
                customKeypad.getKey();
                delay(10);
              }
     ...
     // Return to the File Manager list instead of leaving it entirely --
     // '*' below is the only action that should step back up to Main Menu.
     tilt_tech();
     tft.drawString("FILE MANAGER", 35, 60, 4);
     drawMenu(x, y, TFT_BLACK, TFT_WHITE, edit_Menu, menuCount, selectedIndex, 10);
    }
    ...
    if (customKey == '*')                              // Esc
    {
      Serial.println("BACK KEY: File Manager -> Main Menu");
      delay(500);
      break;
    }
  }
...
void USB() {
  ...
  while (millis() - startWait < 30000) {
    // Check for the back key BEFORE usbhostloop() -- it retries the USB
    // mass-storage mount roughly once a second and can take a noticeable
    // moment when nothing is plugged in, which was delaying the back key.
    customKey = customKeypad.getKey();
    if (customKey == '*') {
      Serial.println("BACK KEY: USB mount screen -> File Manager");
      delay(500);
      usbFlowActive = false;
      return;
    }
...
void usbBrowseFolders() {
  ...
  drawScrollableMenuHeader("USB FOLDERS");
  ...
  while (true) {
    if (needsRedraw) {
      ...
      drawScrollableMenuList(usbFolderList, usbFolderCount, ck1, top1, VISIBLE_ROWS);
      needsRedraw = false;
    }
    ...
    if (customKey == 'B') {
      ...
      Serial.println("BACK KEY: USB folder list -> File Manager");
      delay(500);
      return;
    }
  }
...
void usbBrowseFiles() {
  ...
  drawScrollableMenuHeader("FOLDER FILES");
  ...
  while (true) {
    if (needsRedraw) {
      ...
      drawScrollableMenuList(usbFileList, usbFileCount, ck2, top2, VISIBLE_ROWS);
      needsRedraw = false;
    }
    ...
    if (customKey == 'R') {
      ...
      Serial.println("BACK KEY: USB file list -> USB folder list");
      delay(500);
      return;
    }
  }
...
void usbCopySingleFile(String sourceFileName) {
  ...
  Serial.print("loop() stack high-water mark after single-file copy (bytes free): ");
  Serial.println(uxTaskGetStackHighWaterMark(NULL));
  checkKeypadIntegrity("after single-file copy");
...
bool usbCopyFolder(String folderName) {
  ...
  Serial.print("loop() stack high-water mark after folder copy (bytes free): ");
  Serial.println(uxTaskGetStackHighWaterMark(NULL));
  checkKeypadIntegrity("after folder copy");
...
bool usbCopyOneFile(String folderName, String fileName) {
  ...
  for (unsigned long ttotal = 1; ttotal <= height; ttotal++) {
    ...
    for (size_t m = 0; m < toSend; m++) {
      usbPickBuffer[m] = usbSrcFile.read();
      delayMicroseconds(10);
    }
    uint16_t crc = crc16_ccitt(usbPickBuffer, toSend);
    ...
    while (!cardAcked && retries < 5) {
      ...
      for (size_t m = 0; m < toSend; m++) Serial2.write(usbPickBuffer[m]);
